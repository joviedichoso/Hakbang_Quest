...
const CommunityScreen = ({ navigateToDashboard, navigation }) => {
  ...
  const loadFriendActivities = async (friendId, friendData) => {
    try {
      ...
      if (!activitiesSnapshot.empty) {
        ...
        // No activities found, set default activity
        updateFriendWithActivity(friendId, null, 0);
      }
    } catch (err) {
      ...
      return prevFriends.map((f) => {
        if (f.id === friendId) {
          return {
            ...
            streak: activity ? 1 : 0, // Only set streak if there's an activity
  ...
  const loadLeaderboardData = async () => {
    try {
      ...
      for (const activityDoc of activitiesSnapshot.docs) {
        ...
        // Initialize user distance if not already set
        if (!userDistances[userId]) {
          userDistances[userId] = 0;
        }

        // Ensure distance is a valid number
        const distance = typeof activityData.distance === "number" && !isNaN(activityData.distance) ? activityData.distance : 0;
      ...
      const leaderboardData = userIds.map((userId) => ({
        ...
        distance: userDistances[userId] || 0, // Ensure distance is always a number
  ...
  const formatActivityDescription = (activity) => {
    ...
    const distance = typeof activity.distance === "number" && !isNaN(activity.distance)
                  ...
                  <View style={twrnc`flex-row justify-between mt-2`}>
                    <View style={twrnc`items-center`}>
                      <CustomText weight="semibold" style={twrnc`text-[#FFC107]`}>
                        {typeof selectedFriend.lastActivity.distance === "number" && !isNaN(selectedFriend.lastActivity.distance)
